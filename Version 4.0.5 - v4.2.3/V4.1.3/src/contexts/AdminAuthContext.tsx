...
export const AdminAuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const loginWithDiscord = async (code: string, redirectUri: string) => {
    try {
      ...
      if (data.success) {
        ...
        // Log the session and admin login action
        await logUserSession();
        await logAdminLogin(data.user.username, 'Discord OAuth');
  ...
  const login = async (username: string, password: string) => {
    try {
      ...
      if (data.success) {
        ...
        await logUserSession();
        await logAdminLogin(data.user.username, 'Username/Password');
  ...
  const logAdminLogin = async (username: string, method: string) => {
    try {
      const response = await fetch('https://ipapi.co/json/');
      const locationData = await response.json();
      ...
      await supabase.functions.invoke('admin-logger', {
        body: {
          ...
          message: `Admin "${username}" logged in via ${method}`,
          log_level: 'info',
          username: username,
          action_type: 'login',
  ...
  const logAdminLogout = async (username: string) => {
    try {
      const response = await fetch('https://ipapi.co/json/');
      const locationData = await response.json();
      ...
      await supabase.functions.invoke('admin-logger', {
        body: {
          ...
          message: `Admin "${username}" logged out`,
          log_level: 'info',
          username: username,
          action_type: 'logout',
          database_name: 'admin_panel',
