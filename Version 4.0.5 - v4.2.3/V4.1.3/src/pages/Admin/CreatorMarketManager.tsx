...
const CreatorMarketManager = () => {
  ...
  const handleUpdate = async (id: string, updates: Partial<CreatorRelease>) => {
    try {
      ...
      await logAdminAction(
        ...
        'updated',
        'creator_releases',
        `${release?.creator_name} - ${release?.resource_name}`,
        `Updated creator market release "${release?.resource_name}" by ${release?.creator_name}`
      );
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      // Find the deleted release for logging
      const deletedRelease = releases.find(release => release.id === id);
      ...
      await logAdminAction(
        ...
        'deleted',
        'creator_releases',
        `${deletedRelease?.creator_name} - ${deletedRelease?.resource_name}`,
        `Deleted creator market release "${deletedRelease?.resource_name}" by ${deletedRelease?.creator_name}`
      );
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      await logAdminAction(
        ...
        'added',
        'creator_releases',
        `${newRelease.creator_name} - ${newRelease.resource_name}`,
        `Added creator market release "${newRelease.resource_name}" by ${newRelease.creator_name}`
      );
