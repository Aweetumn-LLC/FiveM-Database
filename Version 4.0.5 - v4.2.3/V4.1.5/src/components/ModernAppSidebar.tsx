      ...
      <SidebarContent className="bg-sidebar-background/50 backdrop-blur-sm">
        ...
        {sidebarGroups.map((group) => (
          state === "expanded" || isMobile ? (
            ...
            // Collapsed state - show just the group icon with tooltip behavior
            <SidebarGroup key={group.id} className="px-2 py-1">
              <SidebarGroupContent>
                <SidebarMenu>
                  <SidebarMenuItem>
                    <SidebarMenuButton 
                      className={`${group.color} p-3 rounded-lg hover:bg-sidebar-hover transition-all duration-200 flex items-center justify-center`}
                      title={group.label}
                    >
