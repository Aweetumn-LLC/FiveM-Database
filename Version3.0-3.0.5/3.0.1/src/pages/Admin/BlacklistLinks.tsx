...
import { AlertCircle, Plus, Trash2, Edit, Shield, Upload } from "lucide-react";
...
const BlacklistLinks = () => {
  ...
  const handleLinksChange = (linksText: string) => {
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="max-w-2xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="text-sm font-medium text-white">Links (Multiple Supported)</label>
              <Textarea
                ...
                rows={4}
              />
              <p className="text-xs text-white/60 mt-1">Enter one link per line - you can add as many links as needed</p>
              {formData.links.length > 0 && (
                <p className="text-xs text-green-400 mt-1">{formData.links.length} link(s) added</p>
              )}
            </div>
