import { BLACKLISTED_CREATORS, IP_VIOLATION_KEYWORDS, COC_VIOLATION_KEYWORDS, IP_BYPASS_KEYWORDS, COC_BYPASS_KEYWORDS, COC_CONTEXTUAL_PATTERNS } from "@/config/blacklistConfig";
...
export type CheckResult = {
  ...
  severity: string;
  matchType: 'exact' | 'keyword' | 'none' | 'discord_id' | 'ip_violation' | 'coc_violation';
...
export const checkResourceCompliance = async (resourceName: string): Promise<ComplianceResult> => {
  const checkResult = checkCreator(resourceName);
...
export const checkCreator = (input: string): CheckResult => {
  ...
  const ipViolationResult = checkIPViolations(processedInput, searchTerm);
  if (ipViolationResult.isBlacklisted) {
    return ipViolationResult;
  }

  // Check for COC violation keywords in the URL/input
  const cocViolationResult = checkCOCViolations(processedInput, searchTerm);
...
const checkIPViolations = (processedInput: string, searchTerm: string): CheckResult => {
  const inputLower = processedInput.toLowerCase();
  const matchedKeywords: string[] = [];
  
  // Check for IP violation keywords
  for (const keyword of IP_VIOLATION_KEYWORDS) {
    const keywordLower = keyword.toLowerCase();
    
    // Check if keyword appears in the input
    if (inputLower.includes(keywordLower)) {
      // Check if any bypass keywords are present
      const hasBypass = IP_BYPASS_KEYWORDS.some(bypass => 
...
const checkCOCViolations = (processedInput: string, searchTerm: string): CheckResult => {
  const inputLower = processedInput.toLowerCase();
  const matchedKeywords: string[] = [];
  const contextualMatches: string[] = [];
  
  // Check for direct COC violation keywords (those without ':' suffix)
  for (const keyword of COC_VIOLATION_KEYWORDS) {
    if (!keyword.endsWith(':')) {
      const keywordLower = keyword.toLowerCase();
      
      if (inputLower.includes(keywordLower)) {
        // Check if any bypass keywords are present
        const hasBypass = COC_BYPASS_KEYWORDS.some(bypass => 
          inputLower.includes(bypass.toLowerCase())
        );
  ...
  const contextualKeywords = COC_VIOLATION_KEYWORDS.filter(k => k.endsWith(':')).map(k => k.slice(0, -1));
  
  for (const pattern of COC_CONTEXTUAL_PATTERNS) {
