...
import { Menu, X, Shield, ChevronDown, Home, Search, BarChart3, Users, FileText, Gavel, HelpCircle, Star, Wrench, Heart, Settings } from "lucide-react";
...
const Header = () => {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-black/90 backdrop-blur-xl border-b border-white/10 safe-area-top">
      <div className="responsive-container py-3 lg:py-4">
        <div className="flex items-center justify-between">
              ...
              {quickLinks.map((link) => (
                link.external ? (
                  <a
                    ...
                    className="text-white/70 hover:text-blue-400 transition-colors duration-300 text-sm lg:text-base font-medium mobile-touch-target"
                  >
                    {link.label}
                  </a>
                ) : (
                  <Link
                    ...
                    className={`flex items-center gap-1 transition-colors duration-300 text-sm lg:text-base font-medium mobile-touch-target ${
                      isActive(link.to!) ? "text-blue-400" : "text-white/70 hover:text-blue-400"
                    }`}
                  >
          ...
          <nav className="hidden xl:flex items-center space-x-1 2xl:space-x-2">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button 
                  variant="ghost" 
                  className="text-white/70 hover:text-white hover:bg-white/10 px-3 py-2 h-auto font-medium text-sm 2xl:text-base rounded-lg transition-all duration-300 group"
                >
              ...
              <DropdownMenuContent className="w-56 bg-black/95 backdrop-blur-xl border-white/20 shadow-2xl">
                <div className="p-2">
                  ...
                  {toolsNavItems.map((item) => (
                    <DropdownMenuItem key={item.to} asChild>
                      <Link 
                        to={item.to} 
                        className="flex items-center space-x-3 text-white/80 hover:text-white hover:bg-white/10 rounded-md px-2 py-2 transition-colors duration-200"
                      >
          ...
          <button
            className="xl:hidden flex items-center justify-center w-10 h-10 text-white/70 hover:text-white hover:bg-white/10 transition-all duration-300 mobile-touch-target rounded-lg"
        ...
        {isMenuOpen && (
          <nav className="xl:hidden mt-6 pb-6 border-t border-white/10 pt-6 mobile-nav-menu rounded-xl bg-black/95 backdrop-blur-xl">
